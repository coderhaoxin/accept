...
function StructType(schema) {
  ...
  function structType(struct, offset, struct1) {
    ...
    if (struct && !(struct instanceof Storage)) {
      for (let p of props) {
        self[p] = struct[p]
      }
    } else if (struct1) {
...
function ArrayType(elementType, length) {
  ...
  function arrayType(data, byteOffset, data1) {
    ...
    setMeta(self, 'length', viewLength)

    if (!opaqueInstance) {
      setMeta(self, 'byteOffset', byteOffset)
      setMeta(self, 'byteLength', elementType._size * viewLength)
...
function structView(Type) {
  return function(storage, offset) {
    return new StructView(Type, storage, offset)
  }
}
